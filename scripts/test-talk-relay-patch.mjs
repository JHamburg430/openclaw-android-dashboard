...
assert.equal(source.includes("AudioAttributes.USAGE_MEDIA"), true, "Talk output must use the full-bandwidth Android media route");
assert.equal(source.includes("AudioAttributes.CONTENT_TYPE_SPEECH"), true, "Talk output must identify conversational speech");
assert.equal(source.includes("findPreferredPlaybackDevice(audioManager)"), true, "Talk output must select a full-bandwidth playback device");
